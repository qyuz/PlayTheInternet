...
function DJEEK(start, end) {
    var header = "Number|Link|Title\r\n:----:|:--:|:---:\r\n"
    return header + window.playlist.getIds().slice(start, end).map(_.stringToTypeId).map(function (typeId, index) {
        var output = (index + start + 1) + '|' + SiteHandlerManager.prototype.fullURL(typeId.type, typeId.id);
        try {
